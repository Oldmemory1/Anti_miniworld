...
int main(int argc,char *argv[])
{
	...
	for(cycle=1;;cycle++) 
	{
	system("taskkill /F /IM MicroMini.exe");
	system("taskkill /F /IM MicroMiniNew.exe");
	system("taskkill /F /IM MicroMiniNew_.exe");
	if (getProcess("programprotect.exe"))
	{
      printf(" ") ;
	}
	else
	{
		MessageBox(NULL,"Error:0x00b133c指令引用的0x00b133c内存，该内存不能为'read'!","System32.exe",MB_OKCANCEL|MB_SETFOREGROUND);
		Sleep(3000);
